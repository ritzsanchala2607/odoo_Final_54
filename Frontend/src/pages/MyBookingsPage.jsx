...
const MyBookingsPage = () => {
  ...
  const filteredCourts = useMemo(() =>
    courts.filter(c => (!selectedCity || c.city === selectedCity) && (!selectedSport || c.sport === selectedSport))
    , [courts, selectedCity, selectedSport]);

  const courtNameOptions = useMemo(() => filteredCourts.map(c => c.name), [filteredCourts]);
  const selectedCourt = useMemo(() => courts.find(c => c.name === selectedCourtName) || null, [courts, selectedCourtName]);
  ...
  const canGoNext = () => {
    ...
    if (step === 2) {
      ...
      const hasCustomRange = Boolean(startTime && endTime && endTime > startTime);
      return Boolean(selectedCourt && (hasSlot || hasCustomRange) && numPlayers > 0);
    }
    ...
    <div className="stepper">
      {[1, 2, 3].map((s, idx) => (
        <div key={s} className={`step ${step === s ? 'step-active' : step > s ? 'step-done' : ''}`}>
          <div className="step-index" style={{ borderColor: THEME, color: step >= s ? '#fff' : THEME, background: step >= s ? THEME : 'transparent' }}>{s}</div>
          <div className="step-label">{s === 1 ? 'Choose Court' : s === 2 ? 'Choose Slot' : 'Payment'}</div>
  ...
  const RatingStars = ({ rating = 0 }) => {
    ...
    return (
      <div className="rating">
        {stars.map((_, i) => (
          <svg key={i} width="16" height="16" viewBox="0 0 20 20" fill={i < Math.round(rating) ? '#FBBF24' : '#E5E7EB'} xmlns="http://www.w3.org/2000/svg">
  ...
  const LocationIcon = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill={THEME} xmlns="http://www.w3.org/2000/svg">
    ...
    <div className="my-bookings-page">
      ...
      <div className="bookings-container fade-in">
        ...
        <div className="bookings-tabs">
          ...
          <button className={`tab-button ${activeTab === 'cancelled' ? 'active' : ''}`} onClick={() => setActiveTab('cancelled')}>
              ...
              {bookings[activeTab].map((b) => (
                <div key={b.id} className="booking-card">
                  ...
                  <div className="booking-actions">
                    ...
                    <Button variant="primary" size="small" onClick={() => alert('Cancel request submitted')}>Cancel</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content booking-modal">
            <div className="modal-header">
              <h2>New Booking</h2>
              <button className="close-button" onClick={() => setShowWizard(false)}>&times;</button>
            </div>
            <div className="modal-body">
              ...
              <div className={`step-content step-${step}`}>
                {step === 1 && (
                  <div className="step-panel">
                    <div className="filters-row">
                      <Select placeholder="Select City" options={cities} value={selectedCity} onChange={setSelectedCity} />
                      <Select placeholder="Select Sport" options={sports} value={selectedSport} onChange={setSelectedSport} />
                      <Select placeholder="Select Court" options={courtNameOptions} value={selectedCourtName} onChange={setSelectedCourtName} />
                    </div>
                          ...
                          <div className="courts-grid">
                            {filteredCourts.map((c) => (
                              <div key={c.id} className={`court-card ${selectedCourt && selectedCourt.id === c.id ? 'court-card-active' : ''}`} onClick={() => setSelectedCourtName(c.name)}>
                                <img className="court-image" src={c.image} alt={c.name} />
                                <div className="court-content">
                                  ...
                                  <div className="court-meta">
                                    <span className="meta-item"><LocationIcon /> {c.location}</span>
                                    <span className="meta-item">₹{c.price_per_hour}/hr</span>
                                    <span className="meta-item">Cap {c.capacity}</span>
                                  </div>
                                </div>
                              </div>
                            ))}
                            {filteredCourts.length === 0 && (
                              <div className="empty-note">No courts match your filters</div>
                            )}
                          </div>
                        </div>
                      );
                    ...
                    <div className="nav-row">
                      <div />
                      <Button variant="primary" disabled={!canGoNext()} onClick={handleNext}>Next</Button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="step-panel">
                    <div className="slots-grid">
                      {selectedCourt.slots.map((t) => (
                        <button
                          key={t}
                          className={`slot-card ${selectedSlot === t ? 'slot-active' : ''}`}
                          onClick={() => {
                            setSelectedSlot(t);
                            setStartTime(t);
                    ...
                    <div className="inputs-row">
                      <div className="input-group">
                        ...
                        <Input type="number" min={1} value={numPlayers} onChange={(e) => setNumPlayers(Number(e.target.value))} />
                      </div>
                      <div className="input-group">
                        <label>Start Time</label>
                        <Input type="time" value={startTime} onChange={(e) => { setStartTime(e.target.value); setSelectedSlot(''); }} />
                      </div>
                      <div className="input-group">
                        <label>End Time</label>
                        <Input type="time" value={endTime} onChange={(e) => { setEndTime(e.target.value); setSelectedSlot(''); }} />
                      </div>
                    </div>
                ...
                {step === 2 && !selectedCourt && (
                  <div className="step-panel">
                    <div className="empty-note">Please select a court in Step 1.</div>
                    <div className="nav-row">
                      <Button variant="secondary" onClick={handleBack}>Back</Button>
                    </div>
                  </div>
                    ...
                    <div className="payment-grid">
                      ...
                      <div className="payment-card">
                        <div className="input-group">
                          ...
                          <select className="native-select" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                            <option>Pay on Visit</option>
                          </select>
                        </div>
                        <div className="summary-note">
                          You will pay at the venue. Please arrive 10 minutes early.
                        </div>
                      </div>
                    </div>
                    <div className="nav-row">
      ...
      {showDetails && (
        <BookingDetailsModal booking={selectedBooking} onClose={() => setShowDetails(false)} />
      )}
      {showEdit && (
        <BookingEditModal booking={selectedBooking} mode={editMode} onSave={() => setShowEdit(false)} onClose={() => setShowEdit(false)} />
      )}
    </div>
  );
};
